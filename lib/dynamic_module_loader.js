...
var DynamicModuleLoader = new Class(
    events.EventEmitter,
    {
        ...
        load:function (moduleName, remoteServerPackageFileExtension, downloadFullUrl, subDirectory)
        {
            ...
            logger.debug("[dynamic-module-loader] Loading " + moduleName + " from " +
            ...
            {
                logger.debug(util.format("[dynamic-module-loader] Remote server package file extension not specified.  Defaulting to '%s'.",
                ...
                function createModuleInstallationDir()
                {
                    fs.access(moduleInstallationDir, fs.constants.R_OK,
                              function (err)
                              {
                                  ...
                                  {
                                      logger.info(util.format("[dynamic-module-loader] Module installation dir %s does not exist.  Creating.", self.settings.moduleInstallationDir));
                                ...
                                fs.access(preinstalled_node_modules_dir, fs.constants.R_OK, function(err)
                                {
                                    if (!err)
                                    {
                                        fs.copy(preinstalled_node_modules_dir, path.join(packageJsonFileLocation, 'node_modules'),
                                            { overwrite: true })
                                            .then(() => installationComplete(0))
                                            .catch(err => logger.error('Error while copying pre-installed dependencies: ' + err));
                                    }
                                    else
                                    {
