...
var DynamicModuleLoader = new JS.Class(
    events.EventEmitter,
    {
        ...
        load:function (moduleName, remoteServerPackageFileExtension, downloadFullUrl, nodeModulesInstallDir)
        {
            ...
            function hopefullyObtainedLock(err)
            {
                ...
                function installDownloadedModule(packageJsonFileLocation)
                {
                    ...
                    {
                        logger.info("Copying " + path.join(nodeModulesInstallationPath, 'node_modules') + " to " + moduleInstallationPath);
        ...
        zipInstalledModule: function(moduleName, nodeModulesInstallDir)
        {
            ...
            logger.info("[dynamic-module-loader] Zipping " + moduleName + " in directory " + path.join(moduleInstallationPath, '..'));
            var zip = spawn(self.settings.zipExecutablePath, ['-r', moduleName + '.zip', moduleName],
                { cwd: path.join(moduleInstallationPath, '..') });
            ...
            zip.on('exit', function (code)
            {
                ...
                {
                    // Success.
                    future.fulfill(undefined, path.join(moduleInstallationPath, '..', moduleName + '.zip'));
                }
                else
                {
