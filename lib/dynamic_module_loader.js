...
var callbacks = VNodeLib.load('callbacks');
...
var DynamicModuleLoader = new JS.Class(
    events.EventEmitter,
    {
        ...
        initialize:function ()
        {
            ...
            this.setNpmExecutablePath(path.normalize('/usr/local/bin/npm'));
            this.setNpmInstallVerbose(false);
            this.setDownloadDir(path.normalize('./downloads'));
            this.setModuleInstallationDir(path.normalize('./installed-modules'));
            this.setModulePackageServerUrl('http://localhost');
            this.setDownloadLockTimeout(30000);
            this.setDefaultRemoteServerPackageFileExtension(TAR_GZIP_SUFFIX);
            this.setUnzipExecutablePath('/usr/bin/unzip');
            this.setNpmSkipInstall(false);
            ...
            this.setLockOwner({id:'DynamicModuleLoader'});
        ...
        getLockDir:function ()
        {
            return this.getLockManager().getLockDir();
        },

        setLockDir:function (newLockDir)
        {
            this.getLockManager().setLockDir(newLockDir);
        },
        ...
        load:function (moduleName, remoteServerPackageFileExtension)
        {
            ...
            if (!remoteServerPackageFileExtension)
            {
                logger.info(util.format("Remote server package file extension not specified.  Defaulting to '%s'.",
                                        self.getDefaultRemoteServerPackageFileExtension()));
                remoteServerPackageFileExtension = self.getDefaultRemoteServerPackageFileExtension();
            }

            self.getLockManager()
                .obtainLock(moduleName, self.getLockOwner(), self.getDownloadLockTimeout())
            ...
            function hopefullyObtainedLock(err)
            {
                ...
                var fullModulePath = path.join(self.getModuleInstallationDir(), moduleName);
                ...
                var moduleDownloadUrl = self.getModulePackageServerUrl() + '/' + moduleCompressedPackageFileName;
                var moduleDownloadTargetPath = path.join(self.getDownloadDir(), moduleCompressedPackageFileName);
                var moduleInstallationPath = path.join(self.getModuleInstallationDir(), moduleName);
                ...
                function startDownload()
                {
                    ...
                    fs.exists(self.getDownloadDir(),
                              function (exists)
                              {
                                  ...
                                  {
                                      logger.debug(util.format("Module download dir %s does not exist.  Creating.", self.getDownloadDir()));
                                      fs.mkdir(self.getDownloadDir(), 0777, true, createModuleInstallationDir);
                                  }
                              });
                }
                ...
                {
                    fs.exists(self.getModuleInstallationDir(),
                              function (exists)
                              {
                                  if (exists)
                                  {
                                  ...
                                  {
                                      logger.debug(util.format("Module installation dir %s does not exist.  Creating.", self.getModuleInstallationDir()));
                                      fs.mkdir(self.getModuleInstallationDir(), 0777, true, wrap(ensureNpmExists));
                                  }
                              });
                }
                ...
                {
                    logger.debug(util.format("Checking for NPM on path %s.", self.getNpmExecutablePath()));
                    fs.exists(self.getNpmExecutablePath(),
                              function (exists)
                              {
                                  if (exists)
                                  {
                                      logger.debug(util.format("NPM found on path %s.", self.getNpmExecutablePath()));
                ...
                function uncompressTarGzFile(downloadedFile)
                {
                    ...
                    var moduleDecompressTargetPath = path.join(self.getDownloadDir(), modulePackageFileName);
                ...
                function extractModulePackageFromTarFile(sourceBundledPackageFilePath)
                {
                    ...
                    fs.createReadStream(sourceBundledPackageFilePath)
                        .pipe(tar.Extract({ path:self.getModuleInstallationDir() }))
                ...
                function installDownloadedModule(moduleInstallationPath)
                {
                    ...
                    else
                    {
                        logger.info(util.format("Installing module %s at location %s.",
                                                ...
                                                moduleInstallationPath));
                        var options = ['install'];
                        if (self.getNpmInstallVerbose())
                        {
                            options.push('--verbose');
                        }
                        var npm = spawn(self.getNpmExecutablePath(), options, {cwd:moduleInstallationPath});
                ...
                {
                    self.getLockManager().releaseLock(moduleName, self.getLockOwner())
                        .when(cleanUp);
        ...
        __decompressZipFile:function (sourceFilePath, targetFilePath)
        {
            ...
            function extractZip()
            {
                var unzip = spawn(self.getUnzipExecutablePath(), [sourceFilePath, '-d', targetFilePath]);
...
$synthesize(DynamicModuleLoader, 'downloadDir');
$synthesize(DynamicModuleLoader, 'moduleInstallationDir');
$synthesize(DynamicModuleLoader, 'modulePackageServerUrl');
$synthesize(DynamicModuleLoader, 'npmExecutablePath');
$synthesize(DynamicModuleLoader, 'npmInstallVerbose');
$synthesize(DynamicModuleLoader, 'npmSkipInstall');
$synthesize(DynamicModuleLoader, 'lockManager');
$synthesize(DynamicModuleLoader, 'downloadLockTimeout');
$synthesize(DynamicModuleLoader, 'lockOwner');
$synthesize(DynamicModuleLoader, 'defaultRemoteServerPackageFileExtension');
$synthesize(DynamicModuleLoader, 'unzipExecutablePath');
