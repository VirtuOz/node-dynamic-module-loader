...
var DynamicModuleLoader = new JS.Class(
    events.EventEmitter,
    {
            ...
            function hopefullyObtainedLock(err)
            {
                ...
                function unlockAndFulfill(err, module)
                {
                    ...
                    if (err)
                    {
                        ...
                        {
                            logger.info(util.format("An error directory for this package (%s) already exists. " +
                                "Will not create a new one.", targetErrorInstallationDir));
                            logger.info(util.format("Deleting %s.", moduleInstallationPath));
                            ...
                            wrench.rmdirRecursive(moduleInstallationPath, false, function(err)
                            {
                                if (err)
                                {
                                    ...
                                    logger.warn(util.format("Error deleting %s. Will continue.", moduleInstallationPath));
                                }
        ...
        __downloadFile:function (sourceUrl, targetFilePath)
        {
            ...
            // To distinguish the case protocole: http:// from the case protocol: file://...
	        ...
	        else if(uri.protocol == 'file:')
	        {
		        ...
		        wr.on("close", function() {
    		        logger.debug(util.format("Finished copying %s", targetFilePath));
		            future.fulfill(undefined, targetFilePath);
  		        });
