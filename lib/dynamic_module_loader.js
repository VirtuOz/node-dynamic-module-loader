    ...
    events.EventEmitter,
    {
        ...
        load:function (moduleName, remoteServerPackageFileExtension, downloadFullUrl, nodeModulesInstallDir)
        {
            ...
            function hopefullyObtainedLock(err)
            {
                ...
                {
                    fs.access(self.settings.moduleInstallationDir, fs.constants.R_OK,
                              function (err)
                              {
                                  if (!err)
                                  {
                                  ...
                                  else
                                  {
                                      ...
                                      fs.mkdir(self.settings.moduleInstallationDir, 0o777, wrap(function()
                                        {
                                            if (nodeModulesInstallDir)
                                            {
                                                createNodeModulesInstallationDir();
                ...
                {
                    fs.access(nodeModulesInstallationPath, fs.constants.R_OK,
                              function (err)
                              {
                                  if (!err)
                                  {
                                  ...
                                  else
                                  {
                                      ...
                                      fs.mkdir(nodeModulesInstallationPath, 0o777, wrap(ensureNpmExists));
                                  }
                              });
                }
                        ...
                        rimraf(path.join(packageJsonFileLocation, 'node_modules'), function(err)
                        {
                            ...
                            else
                            {
                                fs.access(preinstalled_node_modules_dir, fs.constants.R_OK, function(err)
                                {
                                    ...
                                    {
                                        unlockAndFulfill(new Error("[dynamic-module-loader] Cannot find pre-installed node_modules at " +
                                            preinstalled_node_modules_dir), undefined);
                                    }
                                });
                            }
                        });
                                                    ...
                                                    .on('close', function()
                                                        {
                                                            deleteNodeModulesAndRunNpmInstall(function(returnCode) {
                                                                if (returnCode === 0)
                                                                {
                                                                    logger.info("[dynamic-module-loader] Copying " + path.join(nodeModulesInstallationPath, 'node_modules') + " to " + packageJsonFileLocation);
                                                                    fs.copy(path.join(nodeModulesInstallationPath, 'node_modules'), path.join(packageJsonFileLocation, 'node_modules'),
                                                                        { overwrite: true }).then(() => installationComplete(0));
                                                                }
                                                                else
                                                                {
