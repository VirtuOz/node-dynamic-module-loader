...
var DynamicModuleLoader = new JS.Class(
    events.EventEmitter,
    {
        ...
        evict:function(moduleName)
        {
            ...
            function __evict(err, lockName, lockOwner, elapsedTimeMillis)
            {
                var moduleInstallationPath = path.join(self.settings.moduleInstallationDir, moduleName);
            ...
            function __res(err, lockName, actualOwner)
            {
         ...
         * @returns A future object that will be fulfill upon download completed
         */
        ...
        __downloadFile:function (sourceUrl, targetFilePath)
        {
            ...
            var port = url.parse(sourceUrl).port;
            var host = url.parse(sourceUrl).hostname;
            var client = http.createClient(port, host);
            var request = client.request('GET', sourceUrl, { "host": host});
            request.end();
            ...
            request.addListener('response', function (response) {
                ...
                var downloadfile = fs.createWriteStream(targetFilePath, {'flags': 'a'});
                logger.debug(util.format("File size: %s bytes", response.headers['content-length']));

                response.addListener('data', function (chunk) {
                    downloadfile.write(chunk, encoding='binary');
                    // logger.debug(util.format("Writing down a chunk of %s bytes ...", chunk.length));
                });
